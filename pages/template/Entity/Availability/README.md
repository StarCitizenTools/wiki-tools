...
`{{Entity/Availability}}` renders an at-a-glance summary of where an entity can be bought, rented, looted, crafted, mined, harvested, or pledged, followed by detail card(s) of UEX terminal prices. Place it as body content further down the page, separate from the `{{Entity}}` infobox at the top.
...
| `canLoot` | Can loot | boolean | No | Items: derived from the `CanGenerateAsLoot` entity tag. Not used for vehicles or commodities. | Override for the "Loot" summary card. Items only. | `yes` |
...
- A detail card with no UEX prices collapses to a static "No … data in UEX" header rather than disappearing, so the card layout stays stable across entities that do and don't have data.
- Terminal location links disambiguate gateway stations by appending the destination system (`Stanton Gateway` becomes `Stanton Gateway (Pyro)`), since the bare terminal name collides with the destination system's own wiki page.
