...
- **Curate it to differentiate**, not just classify. A bare type ("Human weapons consortium") does not distinguish the company from its peers; include the home location: `<Race> <type> based on <primary HQ>`, e.g. `Human weapons consortium based on Terra`. Keep it concise (~40 chars).
