...
- **Bold**: `**X**` → `'''X'''`. **Italic**: `*X*` → `''X''`.
- **Inline code**: `` `X` `` → `<code><nowiki>X</nowiki></code>`. Wrap the content in `<nowiki>` so the parser renders it literally. This is required, not optional: `<code>` alone does NOT stop MediaWiki parsing its contents, so a template ref like `` `{{Entity}}` `` would otherwise **transclude** (expanding the real template into the doc and polluting the page with its output and categories), `` `[[X]]` `` would become a link, and a `|` would break an enclosing table cell. `<nowiki>` also renders `<`, `>`, and `&` literally, so no separate HTML-escaping is needed — e.g. `` `<blockquote>` `` → `<code><nowiki><blockquote></nowiki></code>` displays the literal tag. (Edge case: if the code content itself contains the literal string `</nowiki>`, fall back to HTML-escaping that fragment, since nowiki cannot nest.)
- **Fenced code blocks**: ` ```lang\n...\n``` ` → `<syntaxhighlight lang="lang">...</syntaxhighlight>`. Bare ` ``` ` (no lang) → `<syntaxhighlight lang="text">...</syntaxhighlight>` — use `lang="text"`, NOT a bare `<syntaxhighlight>`: a language-less tag adds the page to `Pages_with_syntax_highlighting_errors` under MW 1.43's Pygments-based extension, and `text` is the valid no-op lexer (e.g. for an ASCII diagram).
