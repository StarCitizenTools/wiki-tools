...
Use `{{Entity}}` at the top of an item, commodity, or mission page; `{{Vehicle}}` for a ship or ground vehicle; `{{Location}}` for a star system or jump point. All three render through this module and write the page's SMW data, `SHORTDESC`, and categories; nothing else on the page does.

Sibling templates read the same fetch and write no page metadata themselves; place them further down the page: [Template:Entity/Description](https://starcitizen.tools/Template:Entity/Description) (in-game description prose), [Template:Entity/Availability](https://starcitizen.tools/Template:Entity/Availability) (where to buy/rent/loot/craft it), [Template:Entity/Related](https://starcitizen.tools/Template:Entity/Related) (variants or cargo sizes), [Template:Entity/UsedBy](https://starcitizen.tools/Template:Entity/UsedBy) (vehicles that equip this item), [Template:Entity/Blueprints](https://starcitizen.tools/Template:Entity/Blueprints) (crafting recipes), and [Template:Entity/Ports](https://starcitizen.tools/Template:Entity/Ports) (its hardpoint/port tree).
...
- [Template:Vehicle](https://starcitizen.tools/Template:Vehicle): `{{Entity}}` facade scoped to ships and ground vehicles
...
Renderer modules (via `Module:Entity/Data`):
...
- [Module:Entity/Ports](https://starcitizen.tools/Module:Entity/Ports): implements `{{Entity/Ports}}` (no repo template yet)
...
- [Module:Entity/Item](https://starcitizen.tools/Module:Entity/Item): the Item kind + subtype dispatch
- [Module:Entity/Vehicle](https://starcitizen.tools/Module:Entity/Vehicle): the Vehicle kind + family dispatch + sub-builders

No doc page yet:

- [Module:Entity/Location](https://starcitizen.tools/Module:Entity/Location): the Location kind (star systems, jump points)
- [Module:Entity/Location/Util](https://starcitizen.tools/Module:Entity/Location/Util): Location's starmap helpers
...
- [Module:Entity/Facet/*](https://starcitizen.tools/Module:Entity/Registry): the 22 registered facet modules
...
3. `resolveLeaf` refines the matched kind to a subtype leaf via `resolveSubtype`, falling back to Item when nothing matched.
...
7. With no genuine record and an editorial-mode `kind` declared, `apiData` resets to `{}` and the leaf, chain, and enrich hooks re-run from `args` alone (no new fetch).
...
| `getStructuredData` | link, facet | `(ctx) → table` | additive, leaf wins on collision | `StructuredData.store` |
...
| `getCategories` | link | `(ctx) → string[]` | additive | `Categories.build` |
| `getTypeInfo` | link | `(ctx) → table\|nil` | leaf-first wins | `Data.get` |
...
2. Append it to `Registry.kinds` (probe order: cheapest first; `matches()` must still stand alone).
...
3. See [Module:Entity/Item](https://starcitizen.tools/Module:Entity/Item), [Module:Entity/Vehicle](https://starcitizen.tools/Module:Entity/Vehicle), and [Module:Entity/SubtypeResolver](https://starcitizen.tools/Module:Entity/SubtypeResolver) for the shared token-lookup mechanics (Location has no doc page yet).
...
- The four identity-shaped hooks (`matches`, `resolveSubtype`, `getApiConfigs`, `getEditorialManifest`) take no `ctx`; call them with `apiData`/`args` directly.
