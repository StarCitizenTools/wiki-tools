...
**Subtype additions.** Adding a fourth subtype requires: (1) a new leaf module at `pages/module/Entity/Vehicle/<Name>.lua` with `p.parent = 'Entity/Vehicle'`, `p.family` (the family token, the same string the map dispatches on), `getTypeInfo`, `getShortDescription`, and its own `getCategories` for family-specific categories; (2) appending one `token → module path` entry to `VEHICLE_FAMILY_MAP` in `Vehicle.lua`, plus the matching flag check in `deriveFamily`. No change to the section sub-builders is needed; they are data-gated (they render only when the relevant API fields are populated).
...
├── GroundVehicle.lua      # Subtype: getTypeInfo (Ground vehicle / Ground vehicles) +
│                          #   getShortDescription + getCategories (Pledge vehicles)
├── Gravlev.lua            # Subtype: getTypeInfo (Gravlev / Gravlevs) + getShortDescription
│                          #   + getCategories (Pledge vehicles)
