...
Renders an entity's port loadout for items and vehicles. It is a sibling renderer parallel to [Module:Entity/Availability](https://starcitizen.tools/Module:Entity/Availability) and [Module:Entity/Description](https://starcitizen.tools/Module:Entity/Description), and consumes [Module:Entity/Data](https://starcitizen.tools/Module:Entity/Data) so it shares Apiunto's cache with the Entity infobox and any other Entity-family template on the page.
...
The module is a thin coordinator that delegates to three focused sub-modules: `Categories` (config), `Pipeline` (data transformations), and `Render` (wikitext generation). It also pulls in one shared dependency, [Module:Entity/PageResolver](https://starcitizen.tools/Module:Entity/PageResolver), to resolve equipped-item links to canonical wiki pages. The coordinator runs a three-step flow: `process` → resolve equipped links → `render`. The contract crossing the Pipeline → Render boundary is a `groups` array of category cards with aggregated rows.
...
[Module:Entity/PageResolver](https://starcitizen.tools/Module:Entity/PageResolver) lives outside this directory (it is a sibling shared with Entity/Related and Entity/UsedBy), so it does not appear in the tree.

`Render` has no testcases because it returns mw.html strings best verified by inspecting rendered output, not by unit tests.
...
Between `process` and `render`, the coordinator resolves equipped-item links to canonical wiki pages. `pipeline.collectEquippedUuids(groups)` gathers the distinct equipped UUIDs from every row and nested child. [Module:Entity/PageResolver](https://starcitizen.tools/Module:Entity/PageResolver)`.resolve(uuids)` batches them into one pair of `mw.smw.ask` queries; this is the shared resolution seam, also used by Entity/Related and Entity/UsedBy, and the single place to swap if the SMW store is replaced. Finally, `pipeline.applyResolvedLinks(groups, map)` rewrites each row's equipped link in place to point at the resolved page, keeping the friendly item name as the label. Unresolved UUIDs keep their name-based fallback link.
...
The `uuid` argument falls back to the SMW UUID set by `Template:Entity` on a prior parse, so on a typical entity page you invoke the template bare:
...
{
  "categories": {
    "Docked Vehicles": { "order": 1 },
    "Weapons":         { "order": 5, "expandIntoTypes": ["WeaponGun"] },
    "Manned Turrets":  { "order": 10, "expandIntoTypes": ["Turret", "WeaponGun", "MissileLauncher", "TractorBeam"] },
  ...
  "typeAliases": {
    ...
    "JumpDrive":        "Quantum Drives"
  }
}
```

(Abbreviated: the live catalog carries ~38 categories and 7 `typeAliases`.)
...
- **`typeAliases`** maps an API `type` string to a category label, for ports that lack `category_label` (typically children of vehicle ports, where Apiunto only sets `category_label` at the top level). These overrides intentionally diverge from the item-type vocabulary for vehicle-side routing: `WeaponGun → "Weapons"` (the item vocabulary calls them "Guns"), `MissileLauncher → "Missile & Bomb Racks"` (item vocabulary: "Missile racks"), `JumpDrive → "Quantum Drives"` (so jump drives render under the quantum drive's L-tree instead of their own card).
- Unknown labels (a category we haven't catalogued yet) default to a primary card with `order: 999`. This fails open, so new CIG categories render until we sync.
...
- `editable` flag
- The accepted `compatible_types`, so differently-restricted empty slots don't collapse together (e.g. the M4A Cannon's BAR/MEC/POW/VEN attachment mounts)
...
When something is equipped, the pill shows the *equipped item's* size, which is more useful than the port's accepted range. A flex `S1–5` fuel mount with a size-4 tank installed reads as `S4`, not `S1–5`. Only empty ports fall back to the port's range.

## Docked vehicles

For carrier ships, a docking port (a `DockingCollar` with a craft docked) carries the docked vehicle in `attached_vehicle`, while its own `equipped_item` is the docking-tube hardware, whose name is a placeholder (e.g. a raw class_name like `DRAK_Caterpillar_Command_Module_DockingTube`). The Normalize stage renders the *docked vehicle* instead: its clean name, size from `size_class`, and its UUID for canonical-page resolution. The docked craft's own internal ports (fuel / relay / screen mounts) are skipped so the row stays a clean leaf, since the docked ship has its own page for that loadout. These ports route to the `Docked Vehicles` category (`order: 1`), so they sort to the top of the stack.
...
If every top-level port lands in a collapsed category, the module renders only the "Other" card, with no separate empty-state, since users can expand it.
...
- The zero-pad span (`0` in `04×`) carries `aria-hidden="true"` because it's alignment, not data.
- Solo count cells (the `01×` rendered for a count-1 row only when a sibling has count > 1, so its pill stays aligned) carry `aria-hidden="true"` on the whole cell, since the "1×" is structural filler, not information.
...
Every port (the top-level row and every `<li>` in the L-tree) carries the same set of `data-port-*` attrs so any DOM scraper can identify it without parsing the visible text:
...
| `t-entity-ports-count` | Count prefix (`7×`, `4×`, `16×`), monospace and accent color. Omitted entirely when no row in the sibling group exceeds count = 1; aria-hidden when solo. |
...
- **Drill-down per instance.** Not in scope, since the inline structure pattern conveys the loadout shape, and per-hardpoint locations (Front turret, Lower-left turret, etc.) are deemed not part of the player summary. Adding `<details>` back later is additive, not a rewrite.
