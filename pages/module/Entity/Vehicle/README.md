...
The Vehicle kind covers every ship, ground vehicle, and gravlev served by [Apiunto](https://www.mediawiki.org/wiki/Extension:Apiunto)'s `/vehicles/{uuid}` endpoint. `Vehicle.lua` is an orchestrator: it owns kind-level concerns (matching, subtype resolution, structured data, categories, acquisition, header badge, short description) and delegates every infobox section to a per-section sub-builder under `Vehicle/`.
...
- A new cohort stat participates only once it is stored: declare the property in `Module:Entity/properties.json`, write it in `getStructuredData`, add it to the `COHORT_PROPS` table in `ClassStats.lua` (a file-local, not an export), and reference it from the relevant `Profile` axis. A size class with fewer than 5 stored ships (`MIN_COHORT`) also yields no cohort. Existing cohort pages need `forcelinkupdate` before the property is queryable across the cohort.
...
- Deploy `editorial.json` and any new `properties.json` entries before a `Vehicle.lua` change that reads or writes them: `getEditorialManifest` loads `editorial.json` directly, and `StructuredData.store` drops a field absent from `properties.json` instead of storing it, flagging it back as unregistered, so the value is missing until the property catches up.
- `getRelated`'s series (consumed by `{{Entity/Related}}`) is the page's `series` argument; when the invocation carries none, it is read back from [Module:Entity/Store](https://starcitizen.tools/Module:Entity/Store) instead, so `{{Entity/Related}}` needs no argument on a vehicle page. A page whose first link update has not run yet shows the empty state until it does.
