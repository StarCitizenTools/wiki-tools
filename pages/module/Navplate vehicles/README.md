...
- `methodtable.getRows(self, category)`: reads [Module:BucketQuery](https://starcitizen.tools/Module:BucketQuery) for every page in `category` (bare title + Manufacturer), sorted by manufacturer then page, and caches the result per category on the instance. Returns `nil` on a Store failure or an empty result; `make` turns that into an error hatnote.
...
- `methodtable.make(self)`: builds the Ships and Ground vehicles sections, resolves each manufacturer's brand glyph via [Module:Manufacturers](https://starcitizen.tools/Module:Manufacturers), and hands the assembled args to [Module:Navplate](https://starcitizen.tools/Module:Navplate).
