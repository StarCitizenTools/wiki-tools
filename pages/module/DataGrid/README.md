...
- `p.parseSort(raw, columns)`: the `sort` argument (comma-separated keys, each `<label or property> [asc|desc]`, direction defaulting to `asc`) into a list of `{ alias, direction }` in precedence order, or `nil` for an empty `raw`. A name matches a column's alias (`columnAlias`) or its raw property, so it still finds a relabelled column, or `Name` for the lead card; an unrecognized name or direction, or a column named twice, returns a ready-to-display error.
...
- `main` calls `sortRows` (the `Name` alias, byte comparison, unassigned first) right after `runQuery` and before `buildSpecs`, so rows are listed by page title unless `sort` chooses columns; Bucket otherwise returns them in store order.
- `sort` keys only ever name the editor's non-`eyebrow` `columns` lines or the lead card (`Name`); `buildSpecs` matches each against a spec's `label` (the lead card's is `Name`) and sets that spec's `sort` field, which every kind's `buildColDef` (in `Module:AGGridColumns/Kind`) forwards unchanged to AG Grid's own initial-sort key, and its `sortIndex` (the key's 0-based position), which `AGGridColumns.buildColumnDefs` copies onto every column def. `parseSort` excludes `eyebrow` columns from matching, since one is folded into the lead card and never gets its own spec to sort; naming one is the same `no column named` error as naming an unknown property.
