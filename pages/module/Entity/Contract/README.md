...
- "Required" is a contract concept, not a strict runtime one: `Entity.lua` and `Module:Entity/Infobox` guard their hook calls with `if mod.x`, so a missing hook there silently no-ops instead of erroring. But `Module:Entity/Data` calls a kind's `getApiConfigs` and `matches`, and a facet's `matches`, unconditionally with no such guard, so a kind or facet missing one of those identity hooks errors at render instead of no-opping. The conformance test exists to catch either gap before merge.
...
- `editorialMode` is a kind FIELD (checked by `validateFields`), distinct from the `getEditorialManifest` hook that declares which fields a kind exposes; see [Module:Entity/Editorial](https://starcitizen.tools/Module:Entity/Editorial).
