...
- `p.mediaCard(frame)`: the `#invoke` entry point wrapping `renderMediaCard` + `renderMediaBody`; not currently invoked by any template.
...
- On a `stretchLink` card, anything else interactive inside `content`, the readout, or the footer must be lifted above the stretched anchor (`position: relative; z-index: 2` in `styles.css`) or it becomes unclickable.
