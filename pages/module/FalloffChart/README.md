...
- The filled area and its top stroke are `clip-path: polygon(...)` shapes computed in Lua from the vertices (a shape CSS alone cannot derive), stashed whole in a custom property, and consumed as a bare `clip-path: var(...)` value, which passes the [TemplateStyles](https://www.mediawiki.org/wiki/Extension:TemplateStyles) sanitizer. The curve stroke is drawn last so it paints above the guideline layer (floor, gridlines, ticks).
