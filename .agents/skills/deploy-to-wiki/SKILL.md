...
- `testcases.lua` → **skip** — ScribuntoUnit suites run locally only (`mise run test`). The documentation module executes any `Module:<Name>/testcases` page on every render of the module page, so a pushed copy goes stale on the next module change and shows failures that mean nothing. `Module:Commodity/testcases` and `Module:Common/testcases` belong to other editors and are not mirrored here.
