...
- **Number formatting and units** — each numeric column carries a serialisable `format` spec (`{ style = 'number', suffix = ' m' }`, `{ style = 'number', prefix = '$' }`, etc.) that the extension applies client-side via `Intl`. The displayed text regains thousands separators and its unit (`m`, `kg`, `SCU`, `m/s`, `°/s`, `$` for pledge prices, `aUEC` for the in-game average prices), while the underlying value stays a real number, so sort, filter, the set filter, and CSV export keep operating on the number. Crew counts are left unformatted; Vehicle inventory (Stowage) is grouped without a unit because SMW stores no canonical unit for it.
