...
function FloatingUI.renderSection(data)
	if data == nil or type(data) ~= 'table' or next(data) == nil then
		return ''
	end
	...
	local html = mw.html.create(htmlTag):addClass('t-floatingui-section')

	if data['col'] then
		html:addClass('t-floatingui-section--cols-' .. data['col'])
	end

	local dataOrder = { 'label', 'data', 'desc' }
	for _, key in ipairs(dataOrder) do
		if data[key] then
			html:tag(htmlTag):addClass('t-floatingui-' .. key):wikitext(data[key])
		end
	end
...
function FloatingUI.load(frame)
	...
	return frame:extensionTag({
		name = 'templatestyles',
		args = { src = 'Module:FloatingUI/styles.css' },
