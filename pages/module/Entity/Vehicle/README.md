...
- A new cohort stat participates only once it is stored: add the [SMW](https://www.mediawiki.org/wiki/Extension:Semantic_MediaWiki) property, write it in `getStructuredData`, add it to the `COHORT_PROPS` table in `ClassStats.lua` (a file-local, not an export), and reference it from the relevant `Profile` axis. A size class with fewer than 5 stored ships (`MIN_COHORT`) also yields no cohort. Existing cohort pages need `forcelinkupdate` before the property is queryable across the cohort.
