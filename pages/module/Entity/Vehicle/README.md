...
Every sub-builder exposes one `build(apiData, args, ed)` returning an `EntitySectionEntry` or `nil`. They are pure: they never fetch and never re-resolve the subtype.

### Extending

A fourth family: write `pages/module/Entity/Vehicle/<Name>.lua` with `p.parent = 'Entity/Vehicle'`, `p.family` (the token the map dispatches on, and the value editors write to `|family=`), `getTypeInfo`, `getShortDescription`, and its own `getCategories`; add one `token = 'Entity/Vehicle/<Name>'` entry to `VEHICLE_FAMILY_MAP` in `Vehicle.lua`, plus the matching flag check in `deriveFamily`. No sub-builder changes are needed: every section is data-gated and renders only when the relevant API or editorial fields are populated.
...
- A new cohort stat participates only once it is stored: add the SMW property, write it in `getStructuredData`, add it to `ClassStats.COHORT_PROPS`, and reference it from the relevant `Profile` axis. Existing cohort pages need `forcelinkupdate` before the property is queryable across the cohort.
