...
function p.mbox(frame)
	...
	if not title or not text then
		return makeWikitextError('no title or text specified')
	end
...
function p._mbox(title, text, options)
	...
	mbox:attr('role', 'presentation'):addClass('mbox'):addClass(extraclasses)
	...
	mbox:tag('div'):addClass('mbox-text'):wikitext(text)

	return mw.getCurrentFrame():extensionTag({
		name = 'templatestyles',
		args = { src = 'Module:Mbox/styles.css' },
	}) .. tostring(mbox)
end
