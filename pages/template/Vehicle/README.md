...
- A `uuid` that is supplied but doesn't resolve to a genuine vehicle record is not treated as a planned vehicle either: it adds the page to `Pages with an unresolved entity reference` instead of silently rendering one. The page still renders from its wikitext, so a ship whose page goes up before Apiunto has ingested the record needs `family=` exactly as a concept does, until that record lands.
