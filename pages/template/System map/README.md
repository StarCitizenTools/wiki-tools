...
- Left to right is orbital order, not distance. Disc size is a three-tier convention (star, planet, moon), not a measurement, and is scaled against the whole starmap's extents rather than just the bodies on the page, so a given body is the same size on every page it appears on; see [Module:SystemMap](https://starcitizen.tools/Module:SystemMap) for how sizes are mapped.
- What a body is comes from its disc colour, not text: a gas giant reads as a banded amber disc, an ice giant as a banded cyan one, and a type the module doesn't recognise falls back to plain grey.
