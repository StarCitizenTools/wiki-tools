...
--- @field getEditorialManifest nil|fun(): table Editorial-field manifest fragment (field -> { arg, property?, apiPath?, transform?, default? }); fragments merge root-to-leaf, leaf keys win
...
--- @field getRelated nil|fun(ctx: EntityHookContext): EntityRelatedPayload|nil Payload for {{Entity/Related}}; leaf-first, first non-empty wins (Entity/Related resolves with assembly.acceptNonEmpty, so a leaf's nil falls through instead of winning). Base supplies { items = apiData.related_items }
...
--- @field vehicleSeries string|nil The vehicle's editorial series name, rendered as tiles of the other vehicles sharing it, queried from Bucket (Vehicle); nil when absent so getRelated falls through to Base's `items`
