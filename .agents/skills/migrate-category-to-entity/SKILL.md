...
**A no-UUID page is not automatically dead — try to resolve it via the API first.** Some members have an empty `uuid` in their `{{Item}}` block, but the entity often still exists in the API; the page was just created before it was linked, or under a placeholder. Before excluding any page, look it up by name: `https://api.star-citizen.wiki/api/items?filter[name]=<Page name>&locale=en_EN`. If a single record comes back, take its `uuid` and migrate the page normally. When several records share the name (size variants), the `class_name` field disambiguates them (e.g. `RADR_CHCO_S02_BroadSpec_Lite` is the S2, `RADR_CHCO_S03_BroudSpec` the S3) — **trust `class_name`/the API size, not the size stated in the placeholder page's lead, which is often wrong.** Only after an API lookup genuinely returns nothing (items that exist solely in the localization `labels.json`, never as an in-game entity) do you leave the page on `{{Item}}`; those keep a few legacy subcats alive (step 9). Also exclude redirects. Report the *migratable* count, not the raw member count.

**Cross-check membership with SMW — `get-category-members` misses legacy-template pages.** Pages still on an even older template (`{{Item description}}` + `{{Standard on}}`, pre-`{{Item}}`) may not carry the category the way `get-category-members` indexes, so they silently fall out of the worklist (e.g. the Explorer jump module). After listing the category, run `smw-query` for the type (`[[Type::<APIType>]]` or the type's SMW signature) and diff the two member sets; investigate any page present in one but not the other.
...
- **Fix the deprecated `{{Navplate manufacturer|<full name>}}` form when you find it.** Older pages use the singular `{{Navplate manufacturer|Wei-Tek}}` (full manufacturer name); the live template is the plural `{{Navplate manufacturers|<CODE>}}` keyed on the manufacturer code (the same code in the `{{Entity}}` block). Convert it.
...
- **Cite the game build.** These pages are `{{Entity}}`-rendered, i.e. sourced from game/API data, and usually have no other citation. End the lead sentence with one game-data ref:
  `<ref name="ig<patch digits>">{{Cite game|build=[[Star Citizen Alpha <patch>|Alpha <patch>]]|accessdate=<edit date>|text=Datamine}}</ref>`
  e.g. `<ref name="ig480">{{Cite game|build=[[Star Citizen Alpha 4.8.0|Alpha 4.8.0]]|accessdate=2026-05-29|text=Datamine}}</ref>`. Use the **current live patch** (the API's `game_version` tells you — 4.8.0 at time of writing) and the **edit date** for `accessdate`; name the ref `ig<patch digits>` and reuse via `<ref name="ig480" />` if cited again.
  **`|text=Datamine` is not optional.** Without it the template renders "In-game survey", which claims a person observed the item in the game. These pages are built from the API, and nobody has looked. This is the baseline cite for otherwise-uncited game-data prose — it does **not** replace `{{Cite RSI}}` where a real RSI/Spectrum/Galactapedia source exists. The canonical structure (step 4) already carries `== References ==` + `<references />` to render it.
...
- **Check the API for a stat block before assuming a type needs a subtype.** Most backlog types render `generic` (Item base + facets, no code). A dedicated subtype module is warranted only when the API record carries a type-specific stat block worth surfacing (`power_plant`, `cooler`, `shield`, `radar`, `quantum_interdiction_generator`, …). Some types that *sound* mechanical have none — life support generators carry no stat block, so they render generic (Item + Component facet), no subtype. Fetch one record and inspect before writing a module.
- **Realign `types.json` when its name/category disagrees with the live wiki.** `Module:Entity/Item/types.json` maps the API `type` → `{name, category}`; the category must match the actual on-wiki `Category:<Type plural>` or the migrated pages land in a category that doesn't exist. EMP shipped mapped to `EMPs`/`EMP` but the wiki category is `EMP generators` — realigned to `{ "name": "EMP generator", "category": "EMP generators" }` before migrating. Confirm the mapping against the real category page first. (This is a code change — fix it in the same code session that builds/checks the subtype, then deploy, before the content sweep.)
- **A name-borne subdivision belongs in the short description, not the category.** When a type splits into player-visible subdivisions that live in the item *name* (QED/QDMP/QID quantum interdiction generators, gun weapon-types like Laser repeater), surface the subdivision in the SHORTDESC gun-style (a `getShortDescription` override building a synthetic `{name = <subdivision>}` through `formatGradedShortDescription`), but keep the structural **category** on the umbrella type. Subdivisions derived from blurry/capability signals are query-layer facets, not browse buckets — same call as folding the four `Ship.Turret.*` API types into one `Weapon mounts` category.
