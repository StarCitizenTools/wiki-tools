...
- **Filtering** — the categorical columns (manufacturer via the card, type, career, role, size, production state, pledge availability, flight-ready version) use the extension's checkbox **set filter** (`filter = 'aggridSet'`). Numeric columns (including the stacked prices) use the number filter; loaner and concept date use a text filter. The flight-ready version is the `Added in version` SMW Page (the `Update:` patch), shown as the bare version label.
