...
- **The repository README's page catalog is generated.** `mise run docindex` rebuilds it from each page's README (H1 = title, first sentence = summary), and `mise run lint` fails while it is stale or a top-level module or template directory has no README. Never edit the block between the `docindex` markers by hand.
