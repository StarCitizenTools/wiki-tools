...
- `p.toStructuredData(resolved, manifest) → table`: projects each resolved field onto `def.smw`, and appends `Manual API field`, the list of field keys whose source is `fill`/`override`. That [SMW](https://www.mediawiki.org/wiki/Extension:Semantic_MediaWiki) property, together with `hasManualApiData`, drives the `Entities with manual API data` maintenance category so audited fields can be found and retired.
