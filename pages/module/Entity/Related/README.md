...
- `items`: Base's default, `{ items = ctx.apiData.related_items }` (`set_items` / `base_item` / `variant_items`); inherited by every kind that doesn't override it. Rendered as tiles, resolved to wiki pages + `Page Image` in one batched call through [Module:Entity/PageResolver](https://starcitizen.tools/Module:Entity/PageResolver) (shared with UsedBy and Ports).
...
- Set-item display names come from [Module:Entity/Item/types.json](https://starcitizen.tools/Module:Entity/Item/types.json); an unmapped type falls through to the raw API string.
