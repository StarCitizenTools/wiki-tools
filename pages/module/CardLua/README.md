...
- `p.renderHeader(props)` / `p.renderHeaderContent(title, description, eyebrow)`: a static header row (optional eyebrow, title + description, optional `trailing` element on the right). A header with an eyebrow takes the navplate header style of [Module:Entity/Navplates](https://starcitizen.tools/Module:Entity/Navplates): the eyebrow above a larger title that is not painted as a link. `renderHeaderContent` is exposed separately so an interactive consumer (`CollapsibleCard`'s `<summary>`) can place it inside its own header element alongside its own trailing control.
