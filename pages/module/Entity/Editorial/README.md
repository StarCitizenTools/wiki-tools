...
- `p.toStructuredData(resolved, manifest) → table`: projects each resolved field onto `def.property`, and appends `Manual API field`, the list of field keys whose source is `fill`/`override`. That property, together with `hasManualApiData`, drives the `Entities with manual API data` maintenance category so audited fields can be found and retired.
...
- `p.toStoredValue(value) → any`: strips parser strip-markers, rendered HTML, wiki-link markup and stray non-breaking spaces from a string value before it's stored; non-strings pass through untouched. Public because a leaf storing a field itself (no `property` manifest key) needs the same projection.
...
| `property` | for storage | the `Module:Entity/properties.json` key `toStructuredData` projects the value onto; a field with no `property` still resolves but isn't persisted |
| `apiPath` | no | dotted path into `apiData`; presence makes the field an *overlap* field, absence makes it pure-editorial |
| `transform` | no | `number` (strips commas/`K`/`M`/trailing units), `page` (link target), `patchPage` (normalizes a patch reference to its canonical `Update:` page) |
...
- `Module:Entity/Data`'s stored-uuid fallback is suppressed whenever `args.kind` is set, specifically so an editorial page's stale or placeholder stored uuid can't resurrect and defeat editorial mode.
