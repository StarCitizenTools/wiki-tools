...
| `sort` | Sort | string | No |  | Initial sort: a column's label or property, optionally followed by `asc` or `desc` (default `asc`). | `Damage desc` |
...
- Rows are listed by page title unless `sort` chooses a column.
- `sort` sets the grid's initial sort on one `columns` entry (`Property` or `Property desc`; direction defaults to `asc`) instead of leaving the grid unsorted; an unrecognized column name or direction renders as an inline error naming it.
