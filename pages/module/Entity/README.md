...
- The captured build is spelled `version` by the `items`, `vehicles` and `locations` endpoints and `game_version` by `missions`. Read it through `Format.gameVersion(apiData)`; a reader that checks one key shows nothing for every kind served by the other, silently.
