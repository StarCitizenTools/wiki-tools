...
- **`/testcases` pages are not mirrored** — this repository's suites run locally only and `deploy-to-wiki` skips them. A `Module:<Name>/testcases` page appearing on the wiki for a mirrored module is a leftover to delete, not an upstream edit to pull.
