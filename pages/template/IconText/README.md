...
Renders an inline icon and text pair: a small icon followed by a plain text label, neither of which links anywhere. Wraps [Module:IconText](https://starcitizen.tools/Module:IconText); for an icon and label that both link to a page, use [Template:IconLink](https://starcitizen.tools/Template:IconLink) instead.

## Usage

Suited to compact inline metadata such as a clock beside a duration, a map pin beside a location, or a status glyph beside a label. The first positional argument is the text label; `icon` is the file name of the leading glyph. `size` sets the icon size; `class` appends a CSS class to the root span.
...
- The icon is rendered with an empty `link=` and `class=metadata`, so it is not clickable and is excluded from MultimediaViewer, and reads as inert decoration.
