...
local function renderCaption(root, props)
	if not props.hideCaption then
		root:tag('caption'):wikitext(props.caption)
	end
end
		...
		for _, colId in ipairs(sortKeys) do
			...
			if order and order ~= 'none' then
				...
				if colIndex then
					...
					if result ~= 0 then
						return (order == 'asc' and result == -1) or (order == 'desc' and result == 1)
					end
				end
			end
		end
...
function p.render(props)
	...
	root:addClass('t-table'):addClass('wikitable'):addClass(props.class)
	...
	return mw.getCurrentFrame():extensionTag({
		name = 'templatestyles',
		args = { src = 'Module:TableLua/styles.css' },
	}) .. tostring(root)
end
