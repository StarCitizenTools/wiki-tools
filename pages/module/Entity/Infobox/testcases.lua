...
--- @param apiData table
--- @param args table
--- @return EntityHookContext
local function ctx(apiData, args)
	return { apiData = apiData, args = args }
end
...
function suite:testGenericRowsFromApiData()
	local section = buildMetadataSection({}, ctx({ class_name = 'AEGS_Gladius', version = '4.9.0' }, { uuid = 'abc' }))
...
function suite:testChainContributedRowsAreAppended()
	local section = buildMetadataSection({ contributor('Starmap code', 'STANTON') }, ctx({ class_name = 'x' }, {}))
...
function suite:testChainRowAloneKeepsTheSection()
	local section = buildMetadataSection({ contributor('Starmap code', 'NEXUS') }, ctx({}, {}))
...
function suite:testEveryChainLinkContributes()
	...
	local section = buildMetadataSection(chain, ctx({}, {}))
...
function suite:testNilWhenEveryRowIsEmpty()
	self:assertEquals(nil, buildMetadataSection({}, ctx({}, {})))
	self:assertEquals(
		nil,
...
function suite:testEntityTagsRowCarriesTheUuidAsAValueAttribute()
	local content = findItem(
		buildMetadataSection({}, ctx({ entity_tag_map = { { uuid = 'u1', name = 'Ballistic' } } }, {})),
		'Entity tags'
	)
...
function suite:testFooterVerseGuideButton()
	local section = buildFooterSection({}, ctx({}, { verseguideurl = 'https://verseguide.com/location/STANTON' }))
...
function suite:testFooterVerseGuideOrderAfterChainBeforeWikiApi()
	...
	local content = buildFooterSection(chain, ctx({}, args)).content
...
function suite:testFooterNilWithNoButtons()
	...
	self:assertEquals(nil, buildFooterSection({}, ctx({}, { verseguideurl = '' })))
end
