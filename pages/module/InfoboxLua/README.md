...
**Infobox**: `title` (required), `annotation?` (a second name for the same subject, parenthesised after the title and kept out of the container's `aria-label`), `subtitle?`, `image?` (filename string or an Image table; the lone field with auto-discovery, see Gotchas), `imageUploadName?` (overrides the auto-discovery/upload convention base, default `'<page title> - infobox'`), `images?` (Image[], `src` required on each, rendered as tabs), `sections` (Section[]; required, an unguarded `ipairs` over it errors if omitted, though an empty array is fine), `class?`, `css?` (`{ property = value }`).

**Section**: `label?` (a nested subsection with no `label` is silently skipped), `content?` (wikitext), `items?` (Item[]), `sections?` (nested Section[], rendered as tabs, keyed only by `label`), `columns?` (default `1`), `collapsible?` (default `false`; not honoured on a nested subsection), `collapsed?` (default `false`; only takes effect when `collapsible = true`), `class?`.
...
**Image**: `src` (filename; required on every `images[]` entry, auto-discovered only for the top-level `image` field when omitted, see Gotchas), `overlay?` (wikitext), `label?` (tab label), `size?` (px, default `400`), `class?`.
...
- Auto-discovery is a step `getContentHtml` in `InfoboxLua.lua` runs on the single `image` field before validation, probing `'<page title> - infobox.webp'`/`.png`/`.jpg` in that order ([Module:InfoboxLua/ImageResolver](https://starcitizen.tools/Module:InfoboxLua/ImageResolver)); finding none, it falls back to the placeholder image and marks the container `data-gadget-quantumupload-name` (+ `-categories`) for the QuantumUpload gadget's upload control. `images[]` entries skip this step entirely and are validated as given, so a missing `src` there is a hard error, not a fallback.
